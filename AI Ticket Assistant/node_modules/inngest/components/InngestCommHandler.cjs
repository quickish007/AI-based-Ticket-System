...
var InngestCommHandler = class {
	...
	*/
	...
	async handleSyncRequest({ timer, actions, fn, asyncMode, args }) {
		...
		const exeVersion = require_InngestExecution.PREFERRED_CHECKPOINTING_EXECUTION_VERSION;
		...
		const resultHandler = {
			...
			"change-mode": async ({ token }) => {
				switch (asyncMode) {
					case require_types.AsyncResponseType.Redirect: return actions.transformResponse("creating sync->async redirect response", {
						status: 302,
						headers: { [require_consts.headerKeys.Location]: await this.client["inngestApi"]["getTargetUrl"](`/v1/http/runs/${runId}/output?token=${token}`).then((url) => url.toString()) },
						version: exeVersion,
						body: ""
					});
	...
	*/
			...
			if (method === "POST" || forceExecution) {
				...
				try {
					...
					if (rawVersionHeader && Number.isFinite(Number(rawVersionHeader))) {
						const res = require_functions.versionSchema.parse(Number(rawVersionHeader));
				...
				*/
				...
				const handler = {
					...
					"function-resolved": (result$1) => {
						if (forceExecution) {
							const runCompleteOp = {
								id: require_v1._internals.hashId("complete"),
	...
	runStep({ actions, functionId, stepId, data, timer, reqArgs, headers, fn, forceExecution, headerReqVersion }) {
		...
		const immediateFnData = require_functions.parseFnData(data, headerReqVersion);
		let { version: version$1, sdkDecided } = immediateFnData;
		if (version$1 === require_consts.ExecutionVersion.V1 && sdkDecided && fn.fn["shouldOptimizeParallelism"]?.()) version$1 = require_consts.ExecutionVersion.V2;
		const result = require_promises.runAsPromise(async () => {
			...
			const executionOptions = await ((s) => s)({
				[require_consts.ExecutionVersion.V0]: ({ event, events, steps, ctx, version: version$2 }) => {
					...
					return {
						...
						partialOptions: {
							client: this.client,
							runId: ctx?.run_id || "",
							stepMode: require_types.StepMode.Async,
							data: {
								event,
								events,
								runId: ctx?.run_id || "",
				...
				[require_consts.ExecutionVersion.V1]: ({ event, events, steps, ctx, version: version$2 }) => {
					...
					return {
						version: checkpointingConfig && sdkDecided ? require_consts.ExecutionVersion.V2 : version$2,
						partialOptions: {
							client: this.client,
							runId: ctx?.run_id || "",
	...
	*/
	...
	async getResponseSignature(key, body) {
		const now = Date.now();
		return `t=${now}&s=${await require_net.signDataWithKey(body, key, now.toString())}`;
	}
	/**
...
var RequestSignature = class {
	...
	async #verifySignature({ body, signingKey, allowExpiredSignatures }) {
		...
		if (await require_net.signDataWithKey(body, signingKey, this.timestamp) !== this.signature) throw new Error("Invalid signature");
	}
